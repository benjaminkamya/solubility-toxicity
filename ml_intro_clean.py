...
def run_solubility_regression():
    ...
    # Label each point with its compound name
    names = df.loc[y_test.index, "name"]
    for name, actual, pred in zip(names, y_test, predictions):
        plt.annotate(name, (actual, pred), textcoords="offset points",
